...
partition <- expand.grid(seed = 1:100, lambda_1 = seq(0, 1, length.out = 10), lambda_2 = c(0, seq(1e-3, 1e-1, length.out = 13), seq(1e-1, 1, length.out = 7)[2:7]))
...
for(s in 1:100)
{
  ...
  idx <- split(sample(seq_len(n)), 1:5)
  ...
  mse_table <- c(seed, lambda_1, lambda_2, rep(0, 5))
  ...
  for(k in seq_len(5))
  {
    ...
    mse_table[k+3] = mean((valid.x%*%tmp_beta - valid.y)^2)
    
  }
  
  eval(parse(text = paste0('cv.result', file_idx, ' <- rbind(cv.result', file_idx, ', mse_table)')))
